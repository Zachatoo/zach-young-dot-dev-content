...
And that's it! Now you can either create new notes using the quick switcher and be prompted for a note type, or when creating notes from the file explorer the template will be applied without needing to be prompted if creating the note in a folder with a defined type in the [picker template](#picker-template).
